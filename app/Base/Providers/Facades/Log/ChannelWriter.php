...
class ChannelWriter
{
    public const LOG_WALL = '"';
    public const LOG_DELIMITER = ',';
    public const LOG_EOL = "\n";
    ...
    {
        $this->channels = $this->initChannel();
    }

    protected function initChannel(): array
    {
        return [
            'debug' => [
                'path' => 'debug',
    ...
    protected function getHandler($channel, $level, string $filename = 'app'): ChannelStreamHandler
    {
    ...
    public function writeLog($channel, $level, $message, array $context = [])
    {
        ...
        // add custom handler
        $this->channels[$channel]['_instance']->pushHandler($this->getHandler($channel, $level, $filename));
